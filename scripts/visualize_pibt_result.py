...
def show_agents(agents, colors):
    ...
    for agent in agents:
        ...
        plt.plot([tail[0], head[0]], [tail[1], head[1]], '-', c='w', lw=2.0)
        
        plt.plot([subgoal[0]], [subgoal[1]], marker='d', c=col, markersize=8, markeredgecolor='r', markeredgewidth=2)
        rect = mpatches.Rectangle((subgoal[0]-0.15, subgoal[1]-0.15), 0.3, 0.3, linewidth=2, edgecolor='r', facecolor='none')
...
def show_VOCone(agent, color):
    ...
    if "vo" in agent:
        for cone in agent["vo"]:
            ...
            collection = PatchCollection(patches, facecolors=colors, alpha=0.1)
            plt.gca().add_collection(collection)


if __name__ == '__main__': 
    with open(os.path.join(os.getcwd(), "result", "pibt_result.yaml")) as velocity_obstacle:
